...
public static class EffectiveAddressLongAccess
{
    ...
    private static uint EnsureEvenReadAddress(uint address, int frameProgramCounterAdjust, bool isProgramAccess)
    {
        ...
        return (normalized & 1) == 0 ? normalized : throw new AddressErrorException(address, ".l", isRead: true, isProgramAccess: isProgramAccess, frameProgramCounterAdjust: frameProgramCounterAdjust);
    }
    ...
    {
        var normalized = EffectiveAddressMath.NormalizeAddress24(address);
        return (normalized & 1) == 0 ? normalized : throw new AddressErrorException(address, ".l", isRead: false);
    }
}
